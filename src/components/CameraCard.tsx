...
const CameraCard = ({ camera }: CameraCardProps) => {
  ...
  return (
    <motion.div 
      whileHover={{ y: -5 }}
      className="group relative overflow-hidden rounded-xl border-2 border-primary/30 bg-white p-4 shadow-sm hover:shadow-xl transition-all"
    >
      <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-gray-50">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-2">
          <button 
            ...
            className={`rounded-lg p-2 backdrop-blur-md transition-colors ${isWishlist ? 'bg-secondary text-white' : 'bg-white/80 text-secondary hover:bg-secondary/10'}`}
          ...
          <button 
            ...
            className={`rounded-lg p-2 backdrop-blur-md transition-colors ${isOwned ? 'bg-green-500 text-white' : 'bg-white/80 text-green-400 hover:bg-green-50'}`}
      ...
      <div className="mt-4 space-y-2">
        ...
        <Link to={`/camera/${camera.id}`}>
          <Button variant="outline" className="w-full mt-2 rounded-lg border-secondary/30 text-secondary hover:bg-secondary/10 hover:text-secondary">
